...
async def predict_from_coords(request: Request):
    # Read JSON input
    ...
    if marine_resp.status_code != 200:
        return {"error": "Failed to fetch marine data", "status": marine_resp.status_code}
    ...
    if weather_resp.status_code != 200:
        return {"error": "Failed to fetch weather data", "status": weather_resp.status_code}
    ...
    # Return merged features + prediction
